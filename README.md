...
Эти действия должны выполняться с помощью асинхронных запросов (с использованием fetch). Устанавливаемые сервером cookie сохраняются в браузере клиента даже при асинхронном запросе с помощью fetch, поэтому это не будет проблемой для работы с сессиями.
